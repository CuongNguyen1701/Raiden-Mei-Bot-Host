...
module.exports = {
	...
	execute(client, message, args) {
        ...
        if(!user) message.reply('sorry, could not find that user...');
        
        else if( money[user.id].name == "Raiden Mei 2.0#4150") 
        {
            ...
            message.reply("You cannot steal from Mei! Here's a fine of " +  10*steal + currency + "!" );
        }   
        else if(parseInt(args[1]) > steal) message.reply("stop being greedy! you can only steal up to " + steal + currency + "!" );
        
        else if(!money[user.id]) message.reply('sorry that person have no money, they are too poor.' );
        
        else if(parseInt(args[1]) > money[user.id].money) message.reply('they do not have that much money!' );

        else if(parseInt(args[1]) < 1) message.reply("you cannot steal less than 1" + currency + ", that's meaningless!" );

        

        else if( !money[message.author.id] || money[message.author.id].money < 0) message.reply('you are too poor to steal!' );
            ...
            else
            {
                ...
                else
                {
                    ...
                    money[message.author.id].money -= 2*parseInt(args[1]);
                ...
                embed.setTitle(message.author.username + ' failed and received a fine of ' + 2*parseInt(args[1]) + currency + '. ' 
